...
// Embedded assets generated by scripts/build.ts (optional at dev time)
// If the file is missing, we handle via try/catch below
let embeddedAssets: Record<string, { content: string; contentType: string }> | null = null;
try {
    const assetsPath = "./" + "assets.gen.ts"; // avoid literal to keep static analyzers calm
    await Deno.stat(new URL(assetsPath, import.meta.url));
    const mod = await import(assetsPath);
    embeddedAssets = mod.assets as Record<string, { content: string; contentType: string }>;
} catch (_e) {
    embeddedAssets = null;
}
import {
...
async function serveStatic(req: Request): Promise<Response> {
    ...
    try {
        ...
        // If filesystem read fails (e.g., on Deno Deploy), try embedded assets
        const relKey = p.replace(/^\//, "");
        if (embeddedAssets && embeddedAssets[relKey]) {
            const { content, contentType: ct } = embeddedAssets[relKey];
            return new Response(content, { headers: { "content-type": ct, "cache-control": "no-store" } });
        }
